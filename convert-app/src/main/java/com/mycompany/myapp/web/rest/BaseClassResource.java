...
public class BaseClassResource {
	...
	/**
	 * POST /base-classes : Create a new baseClass.
	 *
	 * @param baseClassDTO the baseClassDTO to create
	 * @return the ResponseEntity with status 201 (Created) and with body the new
	 *         baseClassDTO, or with status 400 (Bad Request) if the baseClass has
	 *         already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PostMapping("/base-classes")
	public ResponseEntity<BaseClassDTO> createBaseClass(@RequestBody String Fjson) throws URISyntaxException {
		/*
		 * log.debug("REST request to save BaseClass : {}", baseClassDTO);
		 * 
		 * if (baseClassDTO.getId() != null) { throw new
		 * BadRequestAlertException("A new baseClass cannot already have an ID",
		 * ENTITY_NAME, "idexists"); }
		 */
		BaseClassDTO result = baseClassService.save(Fjson);
